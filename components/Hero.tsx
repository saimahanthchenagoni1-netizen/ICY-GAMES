...
const STRIP_GAMES = GAMES.slice(0, 5);
...
const Hero: React.FC<HeroProps> = ({ onStartGaming }) => {
  return (
    <div className="relative w-full flex flex-col items-center justify-start pt-8 pb-12 px-8">
       {/* TOP CENTER: Floating Game Strip */}
       <div className="relative z-20 mb-16 animate-in slide-in-from-top-10 duration-700 w-full max-w-4xl flex justify-center hidden md:flex">
           ...
           <div className="relative bg-[#0f1016]/80 backdrop-blur-xl border border-white/10 rounded-2xl p-2 flex items-center justify-center gap-2 shadow-2xl">
                {STRIP_GAMES.map((game) => (
                    <div 
                        key={game.id} 
                        className="group relative w-24 h-16 md:w-32 md:h-20 rounded-xl overflow-hidden cursor-pointer transition-transform hover:scale-105 bg-[#1e293b] border border-white/5 flex items-center justify-center" 
                ...
                <div className="hidden md:flex items-center justify-center w-8 h-20 text-gray-500 hover:text-white cursor-pointer transition-colors" onClick={onStartGaming}>
       ...
       <div className="w-full flex flex-col lg:flex-row items-center lg:items-start justify-between gap-12 lg:gap-20 max-w-6xl mt-4">
            <div className="flex-1 flex flex-col items-center lg:items-start text-center lg:text-left space-y-8 animate-in slide-in-from-left-10 duration-700 delay-100">
                <div className="flex flex-col items-center lg:items-start">
                    <div className="relative group cursor-default p-4">
                        <div className="absolute inset-0 bg-cyan-300/20 blur-[80px] rounded-full opacity-60 group-hover:opacity-90 transition-opacity animate-pulse"></div>
                        <div className="flex items-end gap-2 select-none relative z-10">
                            <div className="relative animate-electricity">
                                <svg viewBox="0 0 45 80" className="w-[100px] h-[180px] md:w-[130px] md:h-[220px]">
                                    ...
                                    <path d="M25 0 L0 35 H18 L8 80 L45 30 H25 L35 0 H25 Z" fill="url(#boltGrad)" stroke="#cffafe" strokeWidth="1.5" />
                                </svg>
                            </div>
                            <div className="font-['VT323'] text-[160px] md:text-[220px] leading-[0.6] font-bold text-transparent bg-clip-text bg-gradient-to-b from-white via-cyan-100 to-cyan-500 tracking-tighter filter drop-shadow-[5px_5px_0_#06b6d4] flex transform translate-y-3">
                                <span>C</span><span>Y</span>
                            </div>
                        </div>
                        <div className="absolute -bottom-8 left-2 flex items-center gap-3">
                            <div className="h-[2px] w-12 bg-cyan-400/50"></div>
                            <span className="text-cyan-300 font-['VT323'] text-xl tracking-[0.3em] uppercase glow-text shadow-cyan-500">UNBLOCKED</span>
                            <div className="h-[2px] w-12 bg-cyan-400/50"></div>
                        </div>
                    </div>
                </div>
                <div className="space-y-8 flex flex-col items-center lg:items-start pt-4">
                    ...
                    <button onClick={onStartGaming} className="btn-icy w-64 py-4 bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-bold rounded-xl shadow-[0_0_30px_rgba(6,182,212,0.4)] tracking-wider text-xl uppercase font-['VT323']">
                        START GAMING
                    </button>
                </div>
            </div>
            ...
            <div className="relative group animate-in slide-in-from-right-10 duration-700 delay-200 mt-8 lg:mt-0 hidden lg:block">
                 <div className="absolute -inset-4 bg-gradient-to-tr from-cyan-400/20 to-blue-600/20 rounded-3xl blur-2xl group-hover:blur-3xl transition-all"></div>
                 <div className="relative w-96 bg-[#0f1016] rounded-3xl p-4 border border-white/10 shadow-2xl transform transition-transform group-hover:-translate-y-2 group-hover:scale-105 cursor-pointer" onClick={onStartGaming}>
